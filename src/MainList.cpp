...
void MainList::addNewItem(unsigned int newID, const char *newName,
                            unsigned int newStock, float newPrice)
{
    // agregar item a la lista enlazada
    Item* newItem = createAndSet_atHead(&(this->head), newID, newName,
                                        newStock, newPrice);
...
bool MainList::registerMovement(uint itemID, int movement)
{
	...
	// cada item en la GUI tiene metadata asociada
    // a su puntero en la memoria del programa
